...
func TestUnmarshalJSON(t *testing.T) {
    ...
    "conditions": [
		...
		// N.B. Most pattern parsing tests can be found in types/pattern_test.go
		{
			...
			ast.Permit().When(ast.String("text").Like(types.Pattern{}.Wildcard())),
			testutil.OK,
		},
		{
			"like single literal",
			...
			ast.Permit().When(ast.String("text").Like(types.Pattern{}.Literal("foo"))),
			testutil.OK,
		},
		{
			...
			ast.Permit().When(ast.String("text").Like(types.Pattern{}.Wildcard().Literal("foo"))),
			testutil.OK,
		},
		{
